...
import utils.TestConstantsAPI1812.{LPPJsonMax, apiLPPJson, breathingSpaceJSONAfterBS, penaltyDetailsModelMax, timeToPayJsonAfterTTP}
...
class PenaltyDetailsHttpParserSpec extends SpecBase {
  ...
  "The PenaltyDetailsHttpParser" when {
    ...
    "the http response status is 200 OK and contains relevant LPP and breathing space JSON" should {
      ...
      val httpResponse = HttpResponse(OK, apiLPPJson(LPPJsonMax, breathingSpaceJSONAfterBS, timeToPayJsonAfterTTP).toString)
    ...
    "the http response status is 200 OK but the response is in an invalid format" should {
      ...
      val httpResponse = HttpResponse(OK, apiLPPJson(Json.obj("f" -> "f"), Json.obj("g" -> "g"), Json.obj("f" -> "f")).toString)
